...
fraction = function(logKa.w, dUaw, logKlip.w, logKpro.w) {
  ...
  # mt = mfre + mprot + mlip + mair
  # Ct*Vm = Cdis*Vm + C.lipid*Clip*Vm + C.prot*Cprot*Vm + Cair*Va
  # Ct = Cdis + C.lipid*Clip + Cprot*Cprot + Cair*Va/Vm
  # Clip = Cdis*Klip, Cprot = Cdis*Kprot, Cair = Cdis*Ka
  # Ct = Cdis + Cdis*C.lipid*Klip + Cdis*C.prot*Kprot + Cdis*Ka*Va/Vm
  # Ct = Cdis (1 + C.lipid*Klip + Cprot*Kprot + Ka*Va/Vm)
  # Cdis/Ct = fdis = 1/(1 + C.lipid*Klip + Cprot*Kprot + Ka*Va/Vm)
  
  den <- 1 + 10^(logKlip.w)*C.lipid.L + 10^(logKpro.w)*C.prot.L + 10^(logKa.w.t)*Va/Vm
  f.dis <- 1/den
